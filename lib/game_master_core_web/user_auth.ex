defmodule GameMasterCoreWeb.UserAuth do
  use GameMasterCoreWeb, :verified_routes
  ...
  @remember_me_cookie "_game_master_core_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      GameMasterCoreWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      GameMasterCoreWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule GameMasterCoreWeb.PageLive do
        use GameMasterCoreWeb, :live_view

        on_mount {GameMasterCoreWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{GameMasterCoreWeb.UserAuth, :require_authenticated}] do
  ...
  @doc """
  Plug for routes that require the user to be authenticated via API token.
  """
