defmodule GameMasterCoreWeb.NoteControllerTest do
  ...
  describe "note links" do
    ...
    test "list_links returns empty links for note with no links", %{conn: conn, game: game, note: note} do
      conn = get(conn, ~p"/api/games/#{game.id}/notes/#{note.id}/links")
      response = json_response(conn, 200)
    ...
    test "create_link successfully creates note-character link", %{conn: conn, game: game, note: note, scope: scope} do
      character = character_fixture(scope, %{game_id: game.id})
      
      conn = post(conn, ~p"/api/games/#{game.id}/notes/#{note.id}/links", %{
        "entity_type" => "character",
        "entity_id" => character.id
      })
    ...
    test "create_link with missing entity_type returns error", %{conn: conn, game: game, note: note, scope: scope} do
      character = character_fixture(scope, %{game_id: game.id})
    ...
    test "create_link with invalid entity_type returns error", %{conn: conn, game: game, note: note, scope: scope} do
      character = character_fixture(scope, %{game_id: game.id})
      ...
      response = json_response(conn, 400)
      assert response["error"] == "Invalid entity type. Supported types: character, faction, item, location, quest"
    end
    ...
    test "create_link with non-existent character returns error", %{conn: conn, game: game, note: note} do
      conn = post(conn, ~p"/api/games/#{game.id}/notes/#{note.id}/links", %{
        "entity_type" => "character",
        "entity_id" => 999999
      })
    ...
    test "create_link with cross-scope character returns error", %{conn: conn, game: game, note: note} do
    ...
    test "create_link with unsupported entity type returns error", %{conn: conn, game: game, note: note} do
      conn = post(conn, ~p"/api/games/#{game.id}/notes/#{note.id}/links", %{
        "entity_type" => "faction",
        "entity_id" => 1
      })
    ...
    test "create_link prevents duplicate links", %{conn: conn, game: game, note: note, scope: scope} do
      character = character_fixture(scope, %{game_id: game.id})
    ...
    test "delete_link successfully removes note-character link", %{conn: conn, game: game, note: note, scope: scope} do
      character = character_fixture(scope, %{game_id: game.id})
      ...
      # Delete the link
      conn = delete(conn, ~p"/api/games/#{game.id}/notes/#{note.id}/links/character/#{character.id}")
      assert response(conn, 204)
    end

    test "delete_link with non-existent link returns error", %{conn: conn, game: game, note: note, scope: scope} do
      character = character_fixture(scope, %{game_id: game.id})
    ...
    test "delete_link with invalid entity_type returns error", %{conn: conn, game: game, note: note} do
    ...
    test "delete_link with unsupported entity type returns error", %{conn: conn, game: game, note: note} do
      conn = delete(conn, ~p"/api/games/#{game.id}/notes/#{note.id}/links/faction/1")
      response = json_response(conn, 422)
      assert response["error"] == "Linking notes to faction is not yet supported"
    end

    test "denies access to links for notes in games user cannot access", %{conn: conn, scope: scope} do
      other_user_scope = user_scope_fixture()
      other_game = game_fixture(other_user_scope)
      other_note = note_fixture(other_user_scope, %{game_id: other_game.id})
